...
export function activate(context: vscode.ExtensionContext) {
    ...
    if (typeof navigator === 'object') {    // do not run under node.js
        ...
        // yautjaFs
        if (vscode.workspace.workspaceFolders?.some(f => f.uri.scheme === YautjaFS.scheme) || vscode.workspace.workspaceFile.scheme === YautjaFS.scheme) {
            const fs = enableFs(context);

            fs.seed();
            enableProblems(context);
            enableTasks();
            vscode.commands.executeCommand('vscode.open', vscode.Uri.parse(`yautjafs:/sample-folder/large.ts`));
        }
...
function enableFs(context: vscode.ExtensionContext): YautjaFS {
    const fs = new YautjaFS();
